...
* 以三轮方法执行完成之后，如果被中断了则会执行到 selfInterrupt() 方法
...
* 总共分三步来分析:
  ...
  * 经过上述 addWaiter 方法，生成了一个新的节点，进一步看下acquireQueued() 方法做了什么事情
  ...
  // 判断是否应该被挂起，这个方法就是一直尝试将waitStatus 设置为signal 设置成功了则将线程挂起。
  // 那么signal 是什么状态呢？
      ...
      private void unparkSuccessor(Node node) {
        ...
        // 如果下一个节点为空，或者已经取消等待
          if (s == null || s.waitStatus > 0) {
              s = null;
             // 从尾部开始遍历，一直往前走，知道找到空节点才停下来，对应非空但是已经取消了的还是不处理
