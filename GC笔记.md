...
* JVM 采用了一种RememberSet 来避免全堆扫描（在对引用进行写入的时候，会产生中断，检查refrence 是否来自不同的region/ 新生代，如果是的话，会通过CardTable 在**被引用**的对象的RememberSet 中写入相关的引用信息），在GC 的时候，R-Set 也将成为GC-Root 。
...
* 还有一种情况需要注意，就是当survivor 中**相同年龄的所有对象**的内存之和，大于survivor 总大小的一半之时也会提前进入老年代。
