...
* 二进制文件（binaryLog）：记录了所有对该数据库执行更改操作具体过程。主要用于数据恢复，主备之间的复制，以及操作审计之类的。二级制日志是在事物提交之前进行提交，只写入磁盘一次，不论这时事物大小。

* 重做日志文件：

  只记录与该存储引擎相关的事物日志。重做日志是在事物执行的过程中被不断的被写入，重做日志是innodb 存储引擎写入的日志。
* Undo 日志
