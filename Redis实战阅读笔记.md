...
* 文件事件分为，读事件，写事件，连接事件。
* redis 分为四个部分组成，一个客户端套接字，一个是IO多路复用应用程序，还有事件分发器，事件处理器

1. 客户端发起连接触发事件链接处理器，创建客户端套接字，并且将该套接字的可读事件 与命令执行处理器做关联
2. 当客户端发出命令，触发服务器的可读事件，被可读处理器处理，解析命令，参数，然后执行
3. 与此同时，如果执行产生了命令回复，服务器还会将客户端的可写事件与回复处理器关联起来
4. 在回复处理器写完之后，服务器会接触客户端套接字可写事件与回复处理器之间的关联。
